...
Приравниваем полученные соотношения:
...
\textit{Решение.} $P \left( A \right) = 0$ или $P \left( A \right) = 1 \Rightarrow \forall B, \qquad A$ и $B$ --- независимы.
\begin{enumerate}
\item $P \left( A \right) = 0$.
Тогда $P \left( A \right) P \left( B \right) = 0$.
Вероятность пересечения $P \left( A \cap B \right) \leq \\
\leq P \left( A \right) \Rightarrow P \left( A \cap B \right) = 0$;
\item Рассмотрим случай, когда $P \left( A \right) = 1 \Rightarrow P \left( \overline{A} \right) = 0$.
По предыдущему пункту
$P \left( \overline{A} \cap B \right) =
0 \rightarrow P \left( \overline{A} \cap B \right) =
P \left( B \right) - P \left( A \cap B \right) \Rightarrow \\
\Rightarrow P \left( A \cap B \right) =
P \left( B \right) =
P \left( A \right) \cdot P \left( B \right) =
1 \cdot P \left( B \right) =
P \left( B \right) $.
\end{enumerate}
